...
export default function InstallPrompt() {
  ...
  useEffect(() => {
    if (canInstall && !sessionDismissed) setVisible(true)
  }, [canInstall, sessionDismissed])
    ...
    >
      <div className="w-full max-w-[400px] bg-white rounded-2xl shadow-xl border border-gray-100 p-4 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 leading-tight mt-0.5">Get faster access from your home screen</p>
        </div>

        <div className="flex items-center gap-2 shrink-0">
          <button onClick={handleDismiss} className="text-xs text-gray-400 hover:text-gray-600 transition-colors px-1 py-1">
            Not now
          </button>
          <button onClick={handleInstall} className="bg-green-600 hover:bg-green-700 active:bg-green-800 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors">
            Install
          </button>
        </div>
      </div>
